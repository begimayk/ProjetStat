...
Nous allons effectuer une analyse statistique des données fournies.
Tout d'abord nous pouvons répartir nos données en 2 catégories :
- quantitatives : taille, poids, attaque, block.
- qualitatives : nom, année de naissance, poste, pays de naissance.

Nous avons décidé de commencer par regarder la fréquence des nationalités des joueuses.
Ensuite, nous d'étudier les capacités et aptitudes des joueuses, à savoir la hauteur maximale d'attaque et la hauteur maximale de bloc en fonction de leur poste.
...
```{r}
pays_freq <- table(pays)
pie(pays_freq, labels = names(pays_freq), col = rainbow(length(pays_freq)))
title("Les pays de naissance des joueuses")
par(mar = c(1, 1, 1, 1))
```
