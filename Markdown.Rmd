...
* __annee_naissance__ La date de naissance de la joueuse
...
* __pays__ Le pays de naissance de la joueuse : Brésil, Bulgarie, Chine, Cuba, Égypte, Italie, Japon, Mexique, Perou, Russie, Serbie, Turquie.
...
# Analyse quantitative  univariée
...
* L'axe x - les valeurs de la hauteur d'atttaque
* L'axe y - les fréquences  de 

Sur ce graphique, nous pouvons observer que la distribution de la variable d'attaque ne s'approche pas à la distribution normale.
...
```
La p valeur du test est extrêmement faible, soit 1.619e-13, ce qui indique que nous avons suffisamment de preuves pour rejetter l'hypothèse nulle  H0 selon laquelle les données suivent une distribution normale. Donc la variable ne suit pas la distribution normale
...
#### Moyennes de la hauteur d'attaque de chaque sous groupe (pays)
...
p <- ggplot(moyenne_attaque, aes(x = Pays, y = Moyenne_attaque, fill = Pays)) +
  geom_bar(stat = "identity") +
  labs(title = "Moyenne d'attaque par pays", col= c("#F2B5D4", "#B8E3E6", "#FFDB9A", "#B3D9B2", "#FAC8A7", "#D7D7E0",
 "#F9E6B8", "#B7C9E2", "#F1C2DC", "#C9E8C9", "#FFE4C4", "#D0E0EB"),
 x = "Pays", y = "Moyenne d'attaque") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Incliner les noms des pays en diagonale
...
Nous voyons que la valeur maximale de la hauteur d'attaque appartient à une joueuse de l'Italie.
...
```
Pour les deux tests la p-valeur obtenue est faible, donc nous pouvons rejjeter l'hypothèse nulle selon laquelle la variable suit la loi normale. Nous supposons donc par la suite que ces deux échantillons ne suivent pas une distribution normale.
...
ggplot(df, aes(x = Pays, y = Effectif, fill = Poste)) +
  ...
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
# Incliner les noms des pays en diagonale
````
...
Afin de faire notre analyse mutlivariée, nous étudions d'abord rapidement la hauteur de block.
...
Nous pouvons estimer que la moyenne de block se situe dans l'intervalle [273.4524, 281.4148] avec un niveau de confiance de 95%.
L'estimateur de la moyenne est de 277.4336 cm.
...
lower_bound <- (n - 1) * var(block) / qchisq(1 - alpha/2, df = n - 1)
upper_bound <- (n - 1) * var(block) / qchisq(alpha/2, df = n - 1)
...
De même nous estimons que la variance de la hauteur d'attaque des joueuses se situe dans l'intervalle [465.6286, 742.6398] avec un niveau de confiance de 95%.
L'estimateur de la variance est de 580.0079.
...
```
La proportion de block dont la valeur est supérieure à 280 cm se situe dans l'intervalle [0.4810458, 0.6481979] avec un niveau de confiance de 95%.
La proportion de block dont la valeur est supérieure à 280 cm est de 56.64 %
...
L'histogramme de la repartition des valeurs de la hauteur de block n'est pas symétrique. On pourrais supposer que la variable ne suit pas la distribution normale.

Pour valider notre estimation, nous allons procéder à des tests statistiques.
...
La p valeur du test est extrêmement faible : 9.095e-13, ce qui indique que nous avons suffisamment de preuves pour rejeter l'hypothèse nulle  H0 selon laquelle les données suivent une distribution normale.

## Analyse univariée sur le poste des joueuses

Nous allons commencer par effectuer une analyse du poste des joueuses, qui est une variable _qualitative discrète_.

Nous commençons par un  _tri à plat_.

```{r}

# Calculer les effectifs de chaque pays
effectif_poste <- table(poste)
effectif_poste
...
pie(effectif_poste, paste(names(effectif_poste), "(", formatC(effectif_poste, format = "f", digits = 0), ")", sep = ""), col = pale_colors)

# Ajouter la legende
legend("topright", legend = names(effectif_poste), fill = pale_colors, cex = 0.8)

```

En analysant ce diagramme, nous pouvons constater que les postes des joueuses ont une fréquence bien différente. 

Il est intéressant de noter que les pointues présentent la fréquence maximale dans ce diagramme, avec 48% de joueuses. Cela indique que pointue est le poste le plus représenté parmi les joueuses.


# Analyse multivariée 

## La hauteur du block selon le poste de la joueuse

La hauteur de bloc des joueuses est une variable _quantitative continue_, tandis que leur poste est une variable _qualitative discrète_.

```{r}

tableau <- data.frame(colonne1 = poste, colonne2=block)
...
##### Test de normalité des distribution des sous groupes
...
##### Test d'égalité des variances entre les passeuses et les liberos : 
...
La p-valeur est supérieur au risque alpha de 5%. Nous n'avons pas suffisamment de preuves pour rejetter l'hypothèse nulle H0. Les variances de ces 2 sous groupes sont donc égales.
...
On suppose qu'il existe une relation entre la hauteur d'attaque d'une joueuse et sa taille. Vu que ces deux données sont des variables quantitatives continues nous allons voir s'il existe une relation linéaire qui relie ces 2 variables.
